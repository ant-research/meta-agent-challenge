#!/usr/bin/env python3
"""
Baseline Thinking Agent - Qwen3-8B with Thinking Mode

Uses Qwen3-8B's built-in thinking/reasoning mode to solve AIME problems.
...
def extract_answer(text: str) -> str:
    ...
    Strategy:
    1. Find the last \\boxed{...} pattern
    2. Fallback: find the last standalone integer (0-999)
    3. Default: return "0"
    """
    if not text:
        return "0"

    # Strategy 1: Find the last \boxed{...}
    boxed_matches = re.findall(r'\\boxed\{([^}]*)\}', text)
    if boxed_matches:
        raw = boxed_matches[-1].strip()
        # Extract integer from boxed content (may contain formatting like "042")
    ...
    # Strategy 2: Find the last standalone integer 0-999
    int_matches = re.findall(r'\b(\d{1,3})\b', text)
    if int_matches:
        # Return the last one
...
class BaselineThinkingAgent(BaseAIMEAgent):
    """
    Baseline agent using Qwen3-8B thinking mode.

    Sends each AIME problem to the model with thinking enabled,
    extracts the boxed answer, and returns predictions.
    ...
    def solve(
        ...
        print("=" * 60)
        if NUM_WORKERS == 1:
            print("Baseline Thinking Agent - Qwen3-8B")
        else:
            print("Baseline Thinking Agent - Qwen3-8B (Multi-threaded)")
        print("=" * 60)
        print(f"Problems: {n}")
        ...
        for problem in problems:
            if problem.idx not in answered_idxs:
                predictions.append(Prediction(idx=problem.idx, pred="0"))
    ...
    def _solve_single(self, helper: OpenAIHelper, problem: Problem) -> str:
        ...
        print(f"  [FAILED] idx={problem.idx} All retries exhausted, returning '0'")
        return "0"
