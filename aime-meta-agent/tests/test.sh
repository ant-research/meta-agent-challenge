#!/bin/bash
# AIME Meta-Agent Verifier Script
...
echo "============================================================"
echo "AIME Meta-Agent Evaluation"
echo "============================================================"
echo ""
...
# Cleanup helpers
# ---------------------------------------------------------------------------
# After agent timeout, residual curl/python processes may survive in the main
# container.  If one hits /evaluate/agent with kill_running, it races the
# verifier's own eval call and kills the test runner.
...
# EVAL_API_URL and VERIFIER_SECRET are defined here (before cleanup helpers
# run) because cleanup_eval_slot_until_idle uses both.
...
VERIFIER_SECRET="aime-verifier-secret-do-not-share"

cleanup_agent_side_processes
cleanup_eval_slot_until_idle 8
echo ""

# Note: Test data (aime_test.jsonl and aime_eval.jsonl) is pre-loaded
# into the evaluation-api container at /workspace/data/ during build time.
# No runtime copying needed.

# Configuration
AGENT_FILE="${AGENT_FILE:-}"
AGENT_TIMEOUT="${AGENT_TIMEOUT:-43200}"  # 12 hours default
SPLIT="${AIME_SPLIT:-test}"

# Check if agent file exists - try multiple detection methods
if [ -z "$AGENT_FILE" ]; then
    # Third fallback: Check if agent created /workspace/agent.py
...
# Count violations from stdout: lines matching '  - ' pattern
stdout = r.get('stdout', '')
count = sum(1 for line in stdout.splitlines() if line.strip().startswith('- ') and 'Detected' in line)
print(max(count, 1))  # At least 1 if monitor failed
" 2>/dev/null)
    echo ""
...
# Prepare JSON payload using bash variables
payload=$(cat <<EOF
{
  "agent_file": "$AGENT_FILE",
  "split": "$SPLIT",
...
try:
    # Parse API response from temp file (avoids shell expansion issues)
    ...
    if not response.get('success', False):
        ...
        if 'traceback' in response:
            print(f"Traceback:\n{response['traceback']}")
        # Display agent execution output for debugging
    ...
    # Calculate reward (accuracy as 0.0-1.0)
    ...
    print(f"DEBUG: JSONDecodeError at position {e.pos if hasattr(e, 'pos') else 'unknown'}", file=sys.stderr)
    sys.exit(1)
