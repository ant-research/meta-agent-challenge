...
Unified entry point for running AIME agents.
...
def load_agent_class(agent_path: str) -> Type[BaseAIMEAgent]:
    """
    ...
    Returns:
        Agent class (subclass of BaseAIMEAgent)
    ...
    # Find agent class (subclass of BaseAIMEAgent, not BaseAIMEAgent itself)
    agent_class = None
    for name in dir(module):
        obj = getattr(module, name)
        if (isinstance(obj, type) and
            issubclass(obj, BaseAIMEAgent) and
            obj is not BaseAIMEAgent):
            agent_class = obj
            break
    ...
    if agent_class is None:
        raise ValueError(
            ...
            f"Agent must inherit from BaseAIMEAgent."
        )
...
def run_agent(
    ...
    **agent_kwargs
    ...
    try:
        ...
        problems = BaseAIMEAgent.load_problems(input_file)
        ...
        BaseAIMEAgent.save_predictions(predictions, output_file)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run AIME agent with standardized interface",
...
Examples:
  ...
  python agent_runner.py --agent my_agent.py \\
    ...
    parser.add_argument(
        "--agent",
        required=True,
        help="Path to Python file with agent implementation (must inherit from BaseAIMEAgent)"
    )

    parser.add_argument(
        "--input",
        default="/app/data/aime_eval.jsonl",
        help="Input JSONL file with problems (default: /app/data/aime_eval.jsonl)"
    )
