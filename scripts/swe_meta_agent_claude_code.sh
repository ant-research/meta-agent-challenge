#!/bin/bash
# Run SWE Meta-Agent task with Claude Code agent.
# The meta-agent (claude-code) runs inside the container, builds/optimizes
...
# Model: Opus via Anthropic-compatible endpoint
...
export HARBOR_DEV_DATASET_NAME="${HARBOR_DEV_DATASET_NAME:-swebench-verified@1.0}"
export HARBOR_TEST_DATASET_NAME="${HARBOR_TEST_DATASET_NAME:-swebench-verified@1.0}"
export HARBOR_AGENT_IMPORT_PATH="${HARBOR_AGENT_IMPORT_PATH:-agent:SWEAgent}"
...
export VERIFIER_SECRET="${VERIFIER_SECRET:-swe-verifier-secret-do-not-share}"
...
cleanup() {
    ...
    docker ps -a --filter "name=swe-meta-agent" --format "{{.ID}} {{.CreatedAt}}" | while read container_id created_at; do
