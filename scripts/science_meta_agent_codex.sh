#!/bin/bash
# Test script for science-meta-agent with codex agent
# This script runs a full evaluation using codex as the meta-agent
...
echo "Running science-meta-agent with codex..."
harbor run \
    -p ./science-meta-agent \
    --agent codex \
    --model gpt-5.3-codex \
    --n-concurrent 1 \
    --jobs-dir ./science-meta-agent-codex \
    --force-build \
    --ak version=0.116.0 \
    --yes
