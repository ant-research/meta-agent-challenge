...
HARBOR_MODEL="gpt-5.3-codex"

# Codex 智能体的推理强度参数，这里保持原值。
HARBOR_AGENT_REASONING_EFFORT="xhigh"

# Harbor 单次运行的并发任务数，这里保持原值。
HARBOR_N_CONCURRENT="1"

# Harbor 输出作业目录，这里保持原值。
HARBOR_JOBS_DIR="./meta-agent-lcb-a100-codex"

# Codex/OpenAI 兼容接口的基础地址
export OPENAI_BASE_URL="${OPENAI_BASE_URL:?set in .env}"

# Codex/OpenAI 兼容接口的 API Key
...
echo "[INFO] 使用现有 LiveCodeBench 数据集（跳过 prepare_datasets.py，避免覆盖数据）..."
...
harbor run \
    ...
    --agent "$HARBOR_AGENT" \
    --model "$HARBOR_MODEL" \
    --agent-kwarg "reasoning_effort=$HARBOR_AGENT_REASONING_EFFORT" \
