#!/usr/bin/env python3
"""
SWE Meta-Agent Evaluation Proxy API
...
Env vars:
  ...
  - HARBOR_DEV_DATASET_NAME (required): Harbor dataset to use for development evaluation (e.g. "swe-bench-lite-dev@1.0")
  - HARBOR_TEST_DATASET_NAME (required): Harbor dataset to use for protected test evaluation (e.g. "swe-bench-lite-test@1.0")

  - HARBOR_AGENT_IMPORT_PATH (optional): custom agent import path (default: "agent:SWEAgent")
  - HARBOR_JOBS_DIR (optional): jobs output directory on the cli_service host (default: "/tmp/swe_meta_agent_jobs")
...
_TASK_LIST_DIR = Path(__file__).resolve().parent


def _load_task_list(filename: str) -> list[str]:
    """Load task names from a file in eval_utils/ (one name per line)."""
    fpath = _TASK_LIST_DIR / filename
    if not fpath.exists():
        return []
    return [ln.strip() for ln in fpath.read_text().splitlines() if ln.strip()]


_DEV_TASKS = _load_task_list("dev_tasks.txt")
_TEST_TASKS = _load_task_list("test_tasks.txt")

if not _DEV_TASKS:
    print("[WARNING] dev_tasks.txt is missing or empty — dev evaluation will fail", flush=True)
if not _TEST_TASKS:
    print("[WARNING] test_tasks.txt is missing or empty — test evaluation will fail", flush=True)
...
def _force_release_eval():
    ...
    # Close the dedicated session used by the running _cli_exec call.
    ...
    if sess is not None:
        try:
            ...
            pass
    # Force-release the lock so the next acquire succeeds.
    try:
        _eval_lock.release()
    except RuntimeError:
        pass
...
def _harbor_headers() -> Dict[str, str]:
    headers = {
        "User-Agent": "meta-agent-challenge/swe-meta-agent-eval-proxy",
    }
...
def evaluate_agent():
    ...
    try:
        ...
        agent_import_path = os.environ.get("HARBOR_AGENT_IMPORT_PATH", "").strip() or "agent:SWEAgent"
        ...
        with _eval_busy(nonblocking=True), _periodic_network_prune(interval_sec=300):
            ...
            # Build per-split task include args from task list files.
            split_tasks = _TEST_TASKS if is_verifier else _DEV_TASKS
            if not split_tasks:
                raise RuntimeError(
                    f"{'test' if is_verifier else 'dev'}_tasks.txt is missing or empty; "
                    "cannot determine which tasks to evaluate"
                )
            if first_k is not None and not is_verifier:
                split_tasks = split_tasks[:first_k]
            include_args: list[str] = []
            for tname in split_tasks:
                include_args.extend(["-i", tname])
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="SWE Meta-Agent Evaluation Proxy API")
