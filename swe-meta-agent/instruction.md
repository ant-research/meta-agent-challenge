...
- Downloading SWE-Bench datasets or solutions from the web.
...
Create and iteratively improve a **SWE-Bench solving agent** by writing code in `/workspace`.
...
Implement a Harbor-compatible agent that solves SWE-Bench tasks when executed by the Harbor evaluator.
...
- `HARBOR_AGENT_IMPORT_PATH=agent:SWEAgent` means "import class `SWEAgent` from `agent.py`" in the Harbor CLI working directory (`HARBOR_CLI_CWD`).
...
class SWEAgent(BaseAgent):
    ...
    async def run(self, instruction: str, environment: BaseEnvironment, context: AgentContext) -> None:
        # Use `environment.exec(...)` and other environment helpers to inspect/edit the repo.
        # Populate `context` with any metadata you want to persist.
        #
        # The Harbor evaluator will determine success by running SWE-Bench grading logic
        # against the resulting repository state.
...
The development split contains **250 tasks**. You can pass `"first_k": N` to only evaluate the first N tasks — useful for fast iteration:
...
- The agent import path is configured via `HARBOR_AGENT_IMPORT_PATH` (default: `agent:SWEAgent`).
