#!/usr/bin/env python3
"""
Science Agent Base Class

Defines the standard interface for science meta-agents.
...
@dataclass
class Problem:
    """Science problem data structure (multiple-choice)"""
    idx: int
    question: str
    choices: List[str]  # List of choices in format ["A) ...", "B) ...", ...]
...
class Prediction:
    ...
    idx: int
    pred: str  # Final predicted answer (single letter, e.g. "A")
...
class BaseScienceAgent(ABC):
    """
    Base class for science meta-agents.
    ...
    Usage:
        ...
        problems = load_problems("gpqa_eval.jsonl")
    ...
    def solve(
        ...
        """
        Solve science problems and return predictions.
        ...
        Note:
            ...
            - Answers should be single letters (A, B, C, D, etc.)
        """
        pass
...
def load_problems(filepath: str) -> List[Problem]:
    ...
    return BaseScienceAgent.load_problems(filepath)


def save_predictions(predictions: List[Prediction], filepath: str):
    """Helper function to save predictions"""
    BaseScienceAgent.save_predictions(predictions, filepath)
