#!/usr/bin/env python3
"""
OpenAI API Helper for Science Meta-Agent

Provides utilities for interacting with the OpenAI-compatible API endpoint.
The API connection is pre-configured via environment variables — just
instantiate ``OpenAIHelper()`` and start calling ``chat()``.
...
class OpenAIHelper:
    ...
    that are automatically set by the evaluation system.  You do NOT need to
    pass them yourself — just use ``OpenAIHelper()``.
    """

    def __init__(self):
        """Initialize OpenAI client.

        Reads connection details from environment variables that are
        pre-configured by the evaluation system:
          - TASK_MODEL_API_KEY  — API key
          - TASK_MODEL_API_BASE — API base URL
          - TASK_MODEL_NAME     — model name
        """
        self.api_key = os.environ.get("TASK_MODEL_API_KEY")
        self.base_url = os.environ.get("TASK_MODEL_API_BASE")
        self.model = os.environ.get("TASK_MODEL_NAME")
